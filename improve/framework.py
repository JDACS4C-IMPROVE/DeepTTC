...
def store_predictions_df(params: Dict,
    ...
    if ydf_fpath.exists():
        ...
        if 'Label' in rsp_df.columns:
            rsp_df.columns = [
                x if x != 'Label' else 'auc' for x in rsp_df.columns]
        ...
        mm = mm.astype(
            {params["y_col_name"]: np.float32, pred_col_name: np.float32})
        ...
        df_ = pd.DataFrame({true_col_name: y_true, pred_col_name: y_pred})
...
class ImproveBenchmark(candle.Benchmark):
    ...
    def set_locals(self):
        ...
        if frm_required is not None:
            # This considers global framework required arguments
            self.required.update(set(frm_required))
        if frm_additional_definitions is not None:
            # This considers global framework definitions
            self.additional_definitions.extend(frm_additional_definitions)
